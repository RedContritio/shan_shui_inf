...
class ButtonSet extends React.Component<IProps, IState> {
    ...
    componentDidMount() {
        const updateLeft = (x: number) => this.setState({left: Math.max(4, 40 - x)});
    ...
    render() {
        ...
        return <div id={ButtonSet.id} 
        style={{ backgroundColor: bgrColor, left }} onMouseOver={ onMouseOver } onMouseOut={ onMouseOut } onClick={ onClick } title="settings">
            <div>
                <span id="SET_BTN.t" style={{ fontSize: "large" }}> {icon} </span>
            </div>
        </div>;
    }
}
